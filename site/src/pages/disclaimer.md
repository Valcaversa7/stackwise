---
layout: layouts/page.njk
title: "Disclaimer"
description: "What Stackwise content is, what it is not, and the limits of what we test and verify."
permalink: /disclaimer/
updated: 2026-08-01
eyebrow: Legal
---

<span class="updated">Last updated 1 August 2026</span>

## General information only

Everything published on {{ site.name }} is provided for general informational and educational purposes. It is not professional advice — not legal, financial, medical, security or engineering advice — and it does not create a professional relationship between you and us.

Technology changes quickly. An article that was accurate when published may become outdated. We update articles where we can and date every piece, but you should verify anything critical against primary sources before relying on it.

## Not a substitute for professional advice

Decisions with significant financial, legal or safety consequences — buying a business system, restructuring infrastructure, handling regulated data — should involve a qualified professional who understands your specific situation. We cannot provide that, and nothing on this site should be read as if we did.

## Accuracy and testing

We test what we review, and we describe our methods in the article. But our testing has limits you should know about:

- Hardware results depend on the specific unit we received, its firmware version and our environment. Your unit may differ.
- Software results depend on the version tested. Updates change behaviour.
- Benchmarks measure a workload we chose. They may not represent yours.

Where a result surprised us or contradicted the manufacturer's claim, we say so and show the numbers. Where we did not test something, we say that too.

## External links

Articles link to third-party websites for reference and context. We do not control those sites, are not responsible for their content, and linking to them is not an endorsement. Third-party content may change or disappear after we publish.

## Advertising and affiliate content

{{ site.name }} displays advertising and uses affiliate links. This funding model creates a potential conflict of interest, and we manage it in the open:

- Advertisers have no input into editorial content and no advance sight of it.
- Affiliate links never change a verdict. Our [Editorial Policy](/editorial-policy/) describes the rules we hold ourselves to.
- Affiliate links are labelled in the article and explained in our [Affiliate Disclosure](/affiliate-disclosure/).

If you believe an article was influenced by a commercial relationship, tell us. We will review it and publish the outcome.

## Use at your own risk

Any action you take based on information on this site is strictly at your own risk. To the maximum extent permitted by law, {{ site.name }} will not be liable for any loss or damage arising from the use of our content.

## Corrections

We would rather fix a mistake than defend it. If you spot an error, email [corrections@stackwise.example](mailto:corrections@stackwise.example). Substantive corrections are applied to the article and noted with a date.
